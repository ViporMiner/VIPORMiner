...
```
   ...
   [Важное обновление] Добавить адрес безопасного доступа, который теперь можно настроить через меню - Настройка безопасного адреса доступа, установка этого параметра эффективно предотвращает сканирование фона и предотвращает взрыв 
   Оптимизация проблемы с перекачкой ETC из 3.9.3-3.9.7 
   Полный пакет на китайском, русском и английском языках 
   Добавлены старые оптимизированные переключатели шахт, которые могут отключаться ненужными пользователями 
   Повышение общей стабильности программы 

   3.9.7
   Исправлены ошибки, связанные с ZEPH
   Добавлен SDR
   Переработаны окна редактирования и добавления портов

   3.9.6
   Добавлены валюты GRIN и SPACE
   Глубоко оптимизирована работа с пулами BTC, AntPool и BTC.com
   Исправлены проблемы с отключением от некоторых пулов ZEPH
   Исправлены ошибки, связанные с потерей соединения из-за принудительной передачи успешных шаров
   Оптимизирована работа с алгоритмом kawpow на пуле herominer
   Добавлена настройка кастомного ключа KENC, управляемого RSA128 для сторонних клиентов в разделе Настройки -> Кастомные ключи KENC
   Улучшена защита от атак типа CC (мониторинг сетевых атак)

   3.9.4
   Исправлены проблемы с исчезновением двойных портов TTS
   Улучшена логика проверки пульсации, чтобы избежать случаев оставшихся соединений при экстремальных обстоятельствах, что обеспечивает более стабильную работу программы
   Улучшена обработка высоких задержек и экстремальных условий в пулах
   Добавлен AIPG

   3.9.3
   Добавлен IRON и DOGETHER

   3.9.2
   Добавлен pyi
   Исправлены проблемы с подключением к пулу при использовании IP, SSL и неправильного сертификата

   3.9.1
   Добавлен BEL
   Исправлена проблема с невозможностью работы BLOCX на пуле Mining-Dutch
   Теперь список валют при создании порта можно искать для удобства
   График средней линии теперь отображается в соответствии с настройками среднего значения за час
   Исправлена ошибка в интерфейсе

   3.9.0
   [Важное обновление] Оптимизированы алгоритмы для всех поддерживаемых валют, что увеличило их устойчивость и точность
   Добавлены новые валюты: BLOCX, RTM, RTC, MECU, MAXE, NIKI, SUBI, MEWC, NEVO
   Добавлены порты с поддержкой протоколов TCP/TLS
   Исправлена проблема с повышенным использованием CPU при работе с пулами, использующими протокол TLS и имеющими сетевые флуктуации
   Теперь можно настроить комиссию для всех устройств, связанных с указанным кошельком (в разделе Список устройств -> Список кошельков)
   Обновлены отображаемые данные о хэшрейте: теперь показывается полный хэшрейт, а не хэшрейт после вычета комиссии
   Добавлены графики истории задержки для валют, портов и устройств
   Добавлена средняя линия за 24 часа
   Множество улучшений в интерфейсе
   Оптимизировано отображение хэшрейта для SC-алгоритмов
   ...
   3.8.0
   Добавлены валюты: NEXA, CLORE, NEURAI, NEOXA, ZIL, CLO, UBQ, EGAZ, ELH, AVS, CAU, PAC, PWR, BTN, DUBX, XPB, REDEV2, RTH
   Включена функция группового управления
   Улучшено отображение времени онлайн для устройств
   Отключена капча
   Оптимизирована работа с майнерами, подключенными к пулу okminer
